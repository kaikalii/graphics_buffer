...
fn triangle_contains(a: [f32; 2], b: [f32; 2], c: [f32; 2], point: [f32; 2]) -> bool {
    let b1 = sign(point, a, b) < 0.0;
    let b2 = sign(point, b, c) < 0.0;
    let b3 = sign(point, c, a) < 0.0;
    b1 == b2 && b2 == b3
}
        ...
        F: FnMut(&mut FnMut(&[[f32; 2]])),
    {
        ...
        f(&mut |vertices| {
            for tri in vertices.chunks(3) {
                ...
                for x in tl[0]..br[0] {
                    for y in tl[1]..br[1] {
                        if triangle_contains(tri[0], tri[1], tri[2], [x as f32, y as f32]) {
...
mod tests {
    ...
    fn circle_test() {
        ...
        PNGEncoder::new(File::create("circle.png").unwrap())
            .encode(&*rb, rb.width(), rb.height(), ColorType::RGBA(8))
